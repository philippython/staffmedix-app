  ...
  return (
    <div className={styles.adCard}>
      ...
      <div className={styles.adBody}>
        ...
        <Link to={`/ads/${ad.id}`} className={styles.adCta}>
          Learn More →
        </Link>
      </div>
    </div>
      ...
      <div className={styles.orgCardBody}>
        ...
        {org.description && (
          <p className={styles.orgDesc}>
            {org.description.length > 90
              ? org.description.slice(0, 90) + "…"
              : org.description}
          </p>
        )}
        <div className={styles.orgMeta}>
          {org.size > 0 && (
            <span className={styles.orgChip}>👥 {org.size}+ staff</span>
          )}
          {org.website && <span className={styles.orgChip}>🌐 Website</span>}
          {org.verified && (
            <span className={`${styles.orgChip} ${styles.orgChipGreen}`}>
              ✓ Verified
            </span>
          )}
        </div>
...
export default function AdsPage() {
  ...
  const rawOrgs = orgsData?.results ?? orgsData ?? [];
  ...
  const allOrgs =
    premiumCompanyIds.size > 0
      ? rawOrgs.filter((o) => premiumCompanyIds.has(o.id))
      : rawOrgs;

  // Orgs pagination
  const orgTotalPages = Math.ceil(allOrgs.length / ORGS_PER_PAGE);
  const paginatedOrgs = allOrgs.slice(
    (orgPage - 1) * ORGS_PER_PAGE,
    orgPage * ORGS_PER_PAGE,
  );

  function changeAdPage(p) {
    setAdPage(p);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.hero}>
        <div className={styles.heroInner}>
          <p className={styles.eyebrow}>
            Featured Organisations & Opportunities
          </p>
          ...
          <p className={styles.heroSub}>
            Discover top employers, open roles, and exclusive offers from
            Nigeria's leading healthcare institutions
          </p>
        </div>
      </div>
        ...
        <section className={styles.adsSection}>
          <div className={styles.sectionHead}>
            ...
            <p className={styles.sectionSub}>
              Curated ads from verified healthcare organisations
            </p>
          </div>
      ...
      <section className={styles.orgsSection} ref={orgsRef}>
        <div className={styles.sectionHead}>
          ...
          <p className={styles.sectionSub}>
            Organisations on Pro and Enterprise plans — click any to explore
            their jobs and details
          </p>
        </div>
          ...
          <>
            ...
            {orgTotalPages > 1 && (
              <Pagination
                current={orgPage}
                total={orgTotalPages}
                onChange={changeOrgPage}
              />
            )}
            <p className={styles.orgCount}>
              Showing {(orgPage - 1) * ORGS_PER_PAGE + 1}–
              {Math.min(orgPage * ORGS_PER_PAGE, allOrgs.length)} of{" "}
              {allOrgs.length} organisations
            </p>
          </>
        )}
      </section>
      ...
      <section className={styles.cta}>
        <div className={styles.ctaInner}>
          ...
          <p>
            Upgrade to Pro or Enterprise to reach thousands of qualified
            healthcare professionals
          </p>
          <Link to="/pricing" className={styles.ctaBtn}>
            View Plans →
          </Link>
        </div>
      </section>
