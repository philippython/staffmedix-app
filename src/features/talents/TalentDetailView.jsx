...
export default function TalentDetailView() {
  ...
  const { data: workRaw, isLoading: loadingWork } =
    useGetWorkExperiencesQuery(id);
  const { data: eduRaw, isLoading: loadingEdu } = useGetEducationsQuery(id);
  const { data: skillsRaw } = useGetSkillsQuery(id);
  const { data: credsRaw } = useGetCredentialsQuery(id);

  const work = workRaw?.results ?? workRaw ?? [];
  const edu = eduRaw?.results ?? eduRaw ?? [];
  const skills = skillsRaw?.results ?? skillsRaw ?? [];
  const creds = credsRaw?.results ?? credsRaw ?? [];
    ...
    <div className={styles.page}>
        ...
        <div className={styles.heroInner}>
          ...
          <div className={styles.profileRow}>
            ...
            <div className={styles.avatarWrap}>
              ...
              <div
                className={`${styles.verDot} ${verified ? styles.verDotGreen : styles.verDotGrey}`}
              />
            </div>
            ...
            <div className={styles.identity}>
              ...
              {talent.biography && (
                <p className={styles.tagline}>
                  {talent.biography.length > 180
                    ? talent.biography.slice(0, 180) + "…"
                    : talent.biography}
                </p>
              )}
              <div className={styles.chips}>
                {talent.years_of_experience != null && (
                  <span className={styles.chip}>
                    ...
                    {talent.years_of_experience !== 1 ? "s" : ""} exp
                  </span>
                )}
                {talent.location && (
                  <span className={styles.chip}>📍 {talent.location}</span>
                )}
                {talent.license_number && (
                  <span className={styles.chip}>🪪 Licensed</span>
                )}
              </div>
            </div>
            ...
            <div className={styles.heroBubble}>
              <div
                className={`${styles.bubbleVerDot} ${verified ? styles.bubbleVerGreen : styles.bubbleVerGrey}`}
              />
      ...
      <div className={styles.body}>
                    ...
                    <div key={job.id ?? i} className={styles.timelineItem}>
                      ...
                      <div className={styles.tlContent}>
                        <h3 className={styles.tlTitle}>
                          {job.job_title ?? job.position ?? job.title}
                        </h3>
                        <p className={styles.tlOrg}>
                          {job.facility ?? job.organization ?? job.company}
                        </p>
                        <p className={styles.tlPeriod}>
                          {fmtDate(job.start_date)} –{" "}
                          {job.end_date ? fmtDate(job.end_date) : "Present"}
                        </p>
                        {job.description && (
                          <p className={styles.tlDesc}>{job.description}</p>
                        )}
                      </div>
                    </div>
                  ))}
        ...
        <aside className={styles.rightCol}>
          ...
          {/* Verification */}
          <div
            className={`${styles.card} ${verified ? styles.cardGreen : styles.cardGrey}`}
          >
            ...
            <div className={styles.verRow}>
              ...
              <div>
                <p className={styles.verTitle}>
                  {verified ? "Fully Verified" : "Pending Verification"}
                </p>
          ...
          {skills.length > 0 && (
            <div className={styles.card}>
              ...
              <div className={styles.skillTags}>
                {skills.map((s, i) => (
                  <span key={s.id ?? i} className={styles.skillTag}>
                    {s.name ?? s.skill}
                  </span>
                ))}
              </div>
